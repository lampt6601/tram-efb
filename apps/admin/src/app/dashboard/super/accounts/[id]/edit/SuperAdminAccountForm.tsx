...
import { MAX_PRIORITY_AVAILABLE_ACCOUNTS } from "@thc-efb/shared/account-priority";
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (embedded && onSuccess) {
        ...
        router.push("/dashboard/super/accounts");
      }
    } catch (err: unknown) {
      toast.error(
          ...
          <div className="flex gap-3 border-t border-slate-100 pt-6 dark:border-slate-700">
            ...
            {!embedded && (
              <Button
                type="button"
                variant="outline"
                onClick={() => router.push("/dashboard/super/accounts")}
