          ...
          {seller && (
            <div className="mt-3 rounded-xl border border-indigo-100 bg-indigo-50/30 p-3 dark:border-indigo-500/20 dark:bg-indigo-500/5">
              ...
              {(seller.zaloLink || seller.facebookLink) && (
                <div className="mt-2.5 flex gap-2">
                  {seller.zaloLink && (
                    <a href={seller.zaloLink} target="_blank" rel="noopener noreferrer" className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600">
                      <Image src={zaloIcon} alt="Zalo" className="h-4 w-4 object-contain" />
                      Zalo Người Bán
                    </a>
                  )}
                  {seller.facebookLink && (
                    <a href={seller.facebookLink} target="_blank" rel="noopener noreferrer" className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600">
                      <Image src={facebookIcon} alt="Facebook" className="h-4 w-4 object-contain" />
                      Facebook
                    </a>
                  )}
