...
export function SuperAccountFilters({ totalCount }: { totalCount: number }) {
  ...
  const hasActiveFilters = sort !== "newest" || status !== "all" || search !== "";
  ...
  return (
    <div className={`transition-opacity duration-200 ${isPending ? "opacity-60 pointer-events-none" : ""}`}>
      <div className="flex flex-wrap items-center gap-3">
        ...
        {hasActiveFilters && (
          <Button variant="outline" size="sm" onClick={() => { setLocalSearch(""); startTransition(() => router.replace(pathname, { scroll: false })); }}
