      ...
      <Modal open={open} onClose={onClose}>
        <div className="p-5">
          <div className="mb-4 flex items-start justify-between">
            ...
            <button onClick={onClose} disabled={loading} className="rounded-lg p-1 text-slate-400 hover:bg-slate-100"><X className="h-4 w-4" /></button>
          </div>
          <h2 className="mb-1 text-base font-semibold text-slate-900">Xóa tài khoản admin</h2>
          <p className="text-sm text-slate-500">
            Bạn có chắc muốn xóa admin <span className="font-semibold text-slate-900">{adminEmail}</span>?
          </p>
          {accountCount > 0 && (
